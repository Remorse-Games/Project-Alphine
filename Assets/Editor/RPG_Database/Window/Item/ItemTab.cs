        ...
        public void OnRender(Rect position)
        {
            ...
            GUILayout.BeginArea(new Rect(position.width / 7, 5, tabWidth, tabHeight));
                ...
                GUILayout.BeginArea(new Rect(0, 0, tabWidth, tabHeight));
                    ...
                    if (GUILayout.Button("Change Maximum", GUILayout.Width(firstTabWidth), GUILayout.Height(position.height * .75f / 15 - 10)) && itemSizeTemp > 0)
                    {
                        index = indexTemp = 0;
                        FolderCreator(itemSize, "Assets/Resources/Data/ItemData", "EffectData");
                        ChangeMaximum<ItemData>(itemSizeTemp, item, PathDatabase.ItemExplicitDataPath);

                        //Remove Name Duplicates
                        if (itemSize < itemSizeTemp)
                        {
                            int oldItemSize = itemSize;
